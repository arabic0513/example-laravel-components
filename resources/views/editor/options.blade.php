...
<div class="container">
    ...
    @endphp
    <x-SmartsTable tableId="users" select="{
        style: 'os',
        selector: 'td:first-child'
    }" dom="Bfrtip" :options="['buttons' => $buttons]" getData="{{ route('report') }}" exportId="{{\App\Reports\One::class}}" startDate="{{request()->input('startDate')}}" endDate="{{request()->input('endDate')}}"></x-SmartsTable>
    <x-SmartsTable tableId="users1" select="{
        style: 'os',
        selector: 'td:first-child'
    }" dom="Bfrtip" :options="['buttons' => $buttons]" getData="{{ route('report') }}" exportId="{{\App\Reports\One::class}}" startDate="{{request()->input('startDate')}}" endDate="{{request()->input('endDate')}}"></x-SmartsTable>
</div>
